	...
	public ResponseEntity<Object> prepareSampleData() {
		...
		for(int i = 0; i < 20; i++) {
			productService.saveProduct(
					new Product(
					String.valueOf(89120000 + i),
					"Áo Sơ Mi Cổ Trụ Tay Dài Sợi Modal Ít Nhăn Trơn Dáng Rộng Giá Tốt Non Branded 38 Vol 24", 
					set, 
					"male",
					des, 
					10000000.00, 
...
//		return ResponseEntity.status(200).body(productService.getProductInRange(page, size));
	...
	public ResponseEntity<Object> getProductsByIdCategory(@RequestBody Product product) {
		System.out.println("Product Controller - Get Products By Category");
//		Product product = null;
//		product = 
//		if(rs.toArray().length == 0) {
//			return ResponseEntity.status(409).body(productService.getProductInRange(page, size));
//		}
//		return ResponseEntity.status(200).body(productService.getProductInRange(page, size));
		return ResponseEntity.status(200).body(productService.getProductsByCategory(product.getIdCategory()));
	}
